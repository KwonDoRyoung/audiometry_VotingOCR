...
from .utils.string import is_blank_image, get_characters_width
...
def check_special_char(src):
    ...
    height_dist = np.sum(cv.bitwise_not(src[:, :src.shape[1]//2]), axis=1)
...
class SRT:
    def __init__(self, image: np.ndarray, is_verification:bool, y1:int = 710, y2:int = 870, x1:int = 0, x2:int = 350, padding:int=5, scale_factor:float=4):
    ...
    def crop_region(self, image, y1, y2, x1, x2, padding, scale_factor):
        ...
        # 원본: threshold_w=28000, threshold_h = 65000
        # 4배율: threshold_w=28000*4 = 112000, threshold_h = 65000*4 = 260000
        _cell_bounding_box_list = find_table_grid_points(_temp, 
                                                         n_peak_x = 7 * 2, n_peak_y = 8 * 2,
                                                         threshold_w = 28000 * scale_factor, 
                                                         threshold_h = 65000 * scale_factor)
    ...
    def _ocr_srt(self, x, y_points, w, h):
        _str_list = [] 
        for y in y_points:
            try:
                _dst = crop_by_bounding_box(self.region_image, x, y, w, h, 
                                                x_offset=10, y_offset = 10, 
                                                w_margin = -8, h_margin = -10)
                _dst = cv.morphologyEx(_dst, op=cv.MORPH_DILATE, kernel=np.ones([2, 2]))
                _dst = threshold(_dst, cv.THRESH_BINARY+cv.THRESH_OTSU, blur_size=[7, 7])
                _dst = add_padding(_dst, [10, 0])
                ...
                _str_list.append(_str)
                self.srt_images.append({"image": _dst, "str": _str})
            except Exception as e:
                logger.error(f"Error in processing bounding box ({x}, {y}, {w}, {h}): {e}", exc_info=True)
                raise
    ...
    def _ocr_srt_verification(self, x, y_points, w, h, choice_string_operator, off_char_checker, off_remove_symbol):
        _str_list = [] 
        _percent_list = []
        for y in y_points:
            try:
                ...
                verification = VerificationSRT(cell_image=_dst, choice_string_operator=choice_string_operator, off_char_checker=off_char_checker, off_remove_symbol=off_remove_symbol)
                # print(verification)
                ...
                self.srt_images.append({"image": _dst, "str": _main_str, "srt_candidate": _candidate_str_list})
            except Exception as e:
                logger.error(f"Error in processing bounding box ({x}, {y}, {w}, {h}): {e}", exc_info=True)
                raise
    ...
    def display(self):
        ...
        gs = gridspec.GridSpec(3, 2, figure=fig, wspace=0.2, hspace=0.3)  # 3행 x 2열의 
        ...
        ax4.imshow(self.srt_images[0]["image"], cmap='gray')
        if self.is_verification:
            _candidate = ', '.join(self.srt_images[0]["srt_candidate"])
            ax4.set_title(f'{self.srt_images[0]["str"]}\n{_candidate}')
        else:
            ax4.set_title(f'{self.srt_images[0]["str"]}')
        ax4.axis('on')

        ax5 = fig.add_subplot(gs[2, 1])
        ax5.imshow(self.srt_images[1]["image"], cmap='gray')
        if self.is_verification:
            _candidate = ', '.join(self.srt_images[1]["srt_candidate"])
            ax5.set_title(f'{self.srt_images[1]["str"]}\n{_candidate}')
        else:
            ax5.set_title(f'{self.srt_images[1]["str"]}')
        ax5.axis('on')
    ...
    def __call__(self, choice_string_operator, off_char_checker, off_remove_symbol):
        ...
        if not self.is_verification:
            srt_str = self._ocr_srt(x, [y1, y2], w, h)
            srt_voting_percent = None
        else:
            srt_str, srt_voting_percent = self._ocr_srt_verification(x, [y1, y2], w, h, choice_string_operator=choice_string_operator, off_char_checker=off_char_checker, off_remove_symbol=off_remove_symbol)

        table_data = {"Side": side_str, "SRT": srt_str, "voting_SRT": srt_voting_percent}
        return pd.DataFrame(table_data)

class VerificationSRT:
    ...
    def get_various_images(self, src):
        if len(self.choice_string_operator) == 0:
            return [{"image": src, "str": None}]
        _process_images = []
        _parameters = {"dilate2x2" : {"op": cv.MORPH_DILATE, "kernel": np.ones([2, 2]), "padding":[20, 0]},
        ...
        for _key,_param in _parameters.items():
            if _key in self.choice_string_operator:
                _dst = threshold(src, cv.THRESH_BINARY+cv.THRESH_OTSU, blur_size=[3,3])
                _dst = cv.morphologyEx(_dst, op=_param["op"], kernel=_param["kernel"])
    ...
    def get_char_images(self, src):
        ...
        if self.num_char == 2 and len(_bbox_list) != 2:
            if len(_bbox_list) == 1:
                ...
                logger.error(f"op1-1 : _bbox >= {len(_bbox_list)} 와 num_char =={self.num_char} {_bbox_list}")
                return []
            else:
        ...
        for i, (_x, _y, _w, _h) in enumerate(_bbox_list, start=1):
            ...
            _dst = cv.morphologyEx(_dst, op=cv.MORPH_DILATE, kernel=np.ones([2,2]))
            _dst = threshold(_dst, cv.THRESH_BINARY+cv.THRESH_OTSU, blur_size=[3, 3])        
    ...
    def __call__(self):
        ...
        tesseract_config = {"1-str": {"1th": '--psm 10 --oem 3 -c tessedit_char_whitelist=무05',},
                            ...
                            "3-str": {"1th": "--psm 10 --oem 3 -c tessedit_char_whitelist=05",
                                    "2th": "--psm 10 --oem 3 -c tessedit_char_whitelist=0123456789",
        ...
        for idx, _temp in enumerate(self.process_images):
            ...
            _str = tesseract_ocr(_image, config='--psm 10 --oem 3 -c tessedit_char_whitelist=-0123456789무')
